...
<?php 
				...
				if(isset($submitControl)){
					if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
						...
						$upload = new Upload($_FILES['haber_description']);
						if ($upload->uploaded) { 
								$upload->file_auto_rename = true;
								$upload->process("../uploads/services");
							if ($upload->processed){  
								$haber_description = $upload->file_dst_name;
							} else { 
								$error = Bilgilendirme::Hata("Hay Aksi! Bir hata meydana geldi.".$upload->error);
							}
						}
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert->execute(array($haber_baslik[$key], Seo_Link_Cevir($haber_baslik[$key]), $haber_aciklama[$key], $haber_description, $haber_kisaaciklama[$key], $haber_resim, $key, $haber_durum, $LastID));
									...
									<div class="fileinput fileinput-new" data-provides="fileinput">
										...
										<div>
											<span class="btn btn-primary btn-sm btn-file">
												<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
												<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
												<input type="file" name="haber_description" accept="image/*" id="haber_description">
											</span>
											<a href="#" class="btn btn-primary btn-sm fileinput-exists" data-dismiss="fileinput"><span class="fui-trash"></span>Vazgeç</a>
										</div>
									</div>
					...
					if(isset($submitControl)){
						if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
							$image_info = $db->query("SELECT * FROM ".TABLE." WHERE haber_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
								...
								$upload = new Upload($_FILES['haber_description']);  
								if(!isset($_FILES['haber_description']['name'])) {
									$haber_description = null;
								} else if(!empty($_FILES['haber_description']['name'])) {
									$upload->file_auto_rename = true; 
									$upload->process("../uploads/services");
									if ($upload->processed) {
										$haber_description = $upload->file_dst_name;
									...
									$haber_description = $image_info["haber_description"];
								} 
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									...
									$update->execute(array($haber_baslik[$key], Seo_Link_Cevir($haber_baslik[$key]), $haber_aciklama[$key],  $haber_description, $haber_kisaaciklama[$key],  $haber_resim, $haber_durum, $id, $key));
								}
							}
							if ($update) {
					...
					<div class="col-xl-4 col-xxl-4">
                        <div class="card">
                            <div class="card-body p-3">  
								...
								<label>Resim 870x510</label>
								<small class="form-text text-muted">Seçmiş olduğunuz resim veri içeriğinde kullanılmaktadır.</small> 
								<?php if(empty($row_info["haber_description"])) {?>
										<div class="fileinput fileinput-new" data-provides="fileinput">
											<div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 100%;height: 190px;line-height: 190px;">
												<img src="resim-sec.png" class="img-fluid img-thumbnail" alt="">
											</div> 
								...
								<?php }else{?> 
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/services/<?php echo $row_info["haber_description"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
													<input type="file" name="haber_description" id="haber_description" >
													<div class="ripple-container"></div>
												</span>
												<a href="#" class="btn btn-primary btn-sm fileinput-exists" data-dismiss="fileinput"><span class="fui-trash"></span>Vazgeç</a>
											</div>
